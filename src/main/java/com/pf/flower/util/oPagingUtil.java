...
@AllArgsConstructor
public class oPagingUtil {
	private int omaxNum;//전체 글 개수를 저장하는 변수
	private int opageNum;//현재 보이는 페이지의 번호 저장 변수
	private int olistCnt;//한 페이지 당 보일 게시글의 개수 저장 변수
	private int opageCnt;//보여질 페이지 번호 개수 저장 변수
	private String olistName;//여러개의 게시판이 있을 경우 해당 게시판의
							//url을 저장
	...
	public String makePaging() {
		...
		//전체 게시글 9개 : 1 페이지
		//전체 게시글 11개 : 2 페이지
		int totalPage = (omaxNum % olistCnt) > 0 ?
				omaxNum / olistCnt + 1 :
				omaxNum / olistCnt;
		//2. 현재 페이지가 속해 있는 그룹 번호 구하기
		int curGroup = (opageNum % opageCnt) > 0 ?
				opageNum / opageCnt + 1 :
				opageNum / opageCnt;
		//3. 현재 보이는 페이지 그룹의 시작 번호 구하기
		int start = (curGroup * opageCnt) - (opageCnt - 1);
		//두번째 그룹 시작번호 = pageCnt(5) * 2 - (5 - 1) = 6
		
		//4. 현재 보이는 페이지 그룹의 마지막 번호 구하기
		int end = (curGroup * opageCnt) >= totalPage ?
				totalPage : curGroup * opageCnt;
		
		//이전 버튼 처리
		if(start != 1) {
			sb.append("<a class='pno' href='./" + olistName 
					+ "opageNum=" + (start - 1) + "'>");
			sb.append("&nbsp;이전&nbsp;</a>");
		...
		for(int i = start; i <= end; i++) {
			if(opageNum != i) {//현재 페이지가 아닌 페이지 번호
				sb.append("<a class='pno' href='./" + olistName
						+ "opageNum=" + i + "'>");
				sb.append("&nbsp;" + i + "&nbsp;</a>");
